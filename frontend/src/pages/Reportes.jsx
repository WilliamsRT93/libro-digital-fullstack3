...
/**
 * Modulo para descarga de reportes PDF de notas por alumno.
 * El binario se obtiene desde el BFF y se abre en una pestana nueva.
 */
export default function Reportes() {
  ...
  const descargar = async (e) => {
    ...
    if (!alumnoId || Number(alumnoId) <= 0) {
      setFeedback({ type: "error", msg: "Indica un ID de alumno valido." });
      return;
    }
    ...
    try {
      ...
      // Liberar el objeto URL despues de un breve lapso para que el navegador alcance a abrirlo.
      setTimeout(() => URL.revokeObjectURL(url), 5000);
      setFeedback({ type: "success", msg: "Reporte generado correctamente." });
    } catch (ex) {
      const status = ex.response && ex.response.status;
      const msg = status === 404
        ? "No se encontraron notas para ese alumno."
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--color-text-muted)" }}>
        Genera el reporte PDF con todas las notas del alumno. Una copia se almacena automaticamente en S3.
      </p>
        ...
        <form onSubmit={descargar}>
          ...
          <div style={{ marginTop: 16 }}>
            <button type="submit" disabled={loading}>
              {loading ? <><span className="spinner"></span> Generando...</> : "Descargar PDF"}
            </button>
          </div>
        </form>
      </div>
