...
/**
 * Pagina de login. Valida los campos en el cliente antes de invocar al BFF.
 * Persiste el JWT en sessionStorage al recibir la respuesta exitosa.
 */
export default function Login() {
  ...
  // Si ya hay sesion activa, evitamos volver a mostrar el login.
  if (isAuthenticated()) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (status === 401) {
        setError("Credenciales invalidas. Verifica tu usuario y contrasenia.");
      } else if (status === 503) {
        setError("El API Gateway no esta disponible. Verifica que api-gateway este UP en docker compose ps.");
      } else if (status === 500) {
        setError("Error interno en el servicio. Revisa los logs: docker compose logs ms-auth bff api-gateway.");
      } else if (status >= 500) {
        setError("El servicio de autenticacion no esta disponible. Intenta nuevamente.");
      } else if (!ex.response) {
        setError("No se pudo conectar al BFF (puerto 8080). Verifica que el contenedor 'bff' este corriendo.");
      } else {
        setError(`No se pudo completar el inicio de sesion (HTTP ${status}).`);
      }
    } finally {
      setLoading(false);
    }
        ...
        <form onSubmit={onSubmit} noValidate>
          <div style={{ marginBottom: 14 }}>
            ...
            <input
              ...
              value={form.username}
              onChange={onChange}
              placeholder="docente1 / admin1 / apoderado1"
              disabled={loading}
            />
          </div>
