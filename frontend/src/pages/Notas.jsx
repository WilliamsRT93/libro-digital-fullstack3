...
/**
 * Modulo de captura de notas. La escala es de 1.0 a 7.0.
 */
export default function Notas() {
  ...
  if (!allowed) {
    return <div className="alert error">No tienes permisos para acceder a este modulo.</div>;
  }
  ...
  const onSubmit = async (e) => {
    ...
    if (isNaN(valor) || valor < 1.0 || valor > 7.0) {
      setFeedback({ type: "error", msg: "La nota debe estar entre 1.0 y 7.0" });
      return;
    }
    if (!form.alumnoId || !form.cursoId || !form.asignatura) {
    ...
    try {
      ...
      const msg = status === 403
        ? "No tienes permisos para registrar notas."
    ...
    <div>
        ...
        <form onSubmit={onSubmit}>
          <div className="row">
            <div>
              ...
              <input name="alumnoId" type="number" min="1" value={form.alumnoId} onChange={onChange} required />
            </div>
            <div>
              <label>ID Curso</label>
              <input name="cursoId" type="number" min="1" value={form.cursoId} onChange={onChange} required />
            </div>
            <div>
              <label>Asignatura</label>
              <input name="asignatura" type="text" maxLength={80} value={form.asignatura} onChange={onChange} placeholder="Matematica" required />
            </div>
            <div>
              <label>Nota (1.0 - 7.0)</label>
              <input name="valor" type="number" step="0.1" min="1.0" max="7.0" value={form.valor} onChange={onChange} required />
            </div>
          </div>
          ...
          <div style={{ marginTop: 18 }}>
            <button type="submit" disabled={loading}>
              {loading ? <><span className="spinner"></span> Registrando...</> : "Registrar nota"}
            </button>
          </div>
        </form>
      </div>
      ...
      {registradas.length > 0 && (
        <div>
          ...
          <table>
            <thead>
              <tr><th>Alumno</th><th>Curso</th><th>Asignatura</th><th>Nota</th><th>Descripcion</th></tr>
            </thead>
            <tbody>
              {registradas.map((n) => (
                <tr key={n.id}>
