...
/**
 * Modulo de gestion de usuarios. Solo accesible por ADMIN.
 * Permite listar, crear y eliminar usuarios con roles DOCENTE / APODERADO / ADMIN.
 */
export default function Usuarios() {
  ...
  if (!isAdmin) {
    return <div className="alert error">Solo el rol ADMIN puede gestionar usuarios.</div>;
  }
  ...
  const cargar = async () => {
    try {
      ...
      setFeedback({ type: "error", msg: "No se pudo obtener la lista de usuarios." });
    }
  };
  ...
  const onSubmit = async (e) => {
    ...
    if (form.username.length < 3 || form.password.length < 6 || form.fullName.length < 3) {
      setFeedback({ type: "error", msg: "Completa todos los campos (password >= 6, otros >= 3)." });
      return;
    }
    setLoading(true);
    try {
  ...
  const eliminar = async (id, username) => {
    ...
    try {
      ...
      setFeedback({ type: "error", msg: "No se pudo eliminar el usuario." });
    }
  };
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--color-text-muted)" }}>
        Solo el administrador puede crear docentes y apoderados.
      </p>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <form onSubmit={onSubmit}>
          <div className="row">
            <div>
              <label>Usuario</label>
              <input name="username" value={form.username} onChange={onChange} required minLength={3} maxLength={80}/>
            </div>
            <div>
              <label>Nombre completo</label>
              <input name="fullName" value={form.fullName} onChange={onChange} required minLength={3} maxLength={120}/>
            </div>
            <div>
              <label>Contrasenia (>= 6)</label>
          ...
          <div style={{ marginTop: 18 }}>
            <button type="submit" disabled={loading}>
              {loading ? <><span className="spinner"></span> Creando...</> : "Crear usuario"}
            </button>
          </div>
        </form>
      </div>
      ...
      <table>
        <thead>
          <tr><th>ID</th><th>Usuario</th><th>Nombre</th><th>Roles</th><th>Estado</th><th>Acciones</th></tr>
        </thead>
        <tbody>
          {usuarios.map((u) => (
            <tr key={u.id}>
              ...
              <td>
                <button onClick={() => eliminar(u.id, u.username)} style={{ background: "var(--color-error)" }}>
                  Eliminar
                </button>
              </td>
            </tr>
          ...
          {usuarios.length === 0 && (
            <tr><td colSpan={6} style={{ textAlign: "center", color: "var(--color-text-muted)" }}>Sin usuarios.</td></tr>
          )}
        </tbody>
      </table>
    </div>
