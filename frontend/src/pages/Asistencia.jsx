...
/**
 * Modulo para registrar asistencia. Solo accesible por DOCENTE o ADMIN.
 * Mantiene un historial local en sesion para feedback inmediato al usuario.
 */
export default function Asistencia() {
  ...
  if (!allowed) {
    return (
      <div className="alert error">No tienes permisos para acceder a este modulo.</div>
    );
  }
  ...
  const onSubmit = async (e) => {
    ...
    if (!form.alumnoId || !form.cursoId) {
      setFeedback({ type: "error", msg: "Debes indicar alumno y curso." });
      return;
    }
    ...
    try {
      ...
      const msg = ex.response && ex.response.data && ex.response.data.message
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--color-text-muted)" }}>
        Las inasistencias generan automaticamente una notificacion al apoderado via Kafka.
      </p>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <form onSubmit={onSubmit}>
          <div className="row">
            <div>
              <label htmlFor="alumnoId">ID Alumno</label>
              <input id="alumnoId" name="alumnoId" type="number" min="1" value={form.alumnoId} onChange={onChange} required />
            </div>
            <div>
              <label htmlFor="cursoId">ID Curso</label>
              <input id="cursoId" name="cursoId" type="number" min="1" value={form.cursoId} onChange={onChange} required />
            </div>
            <div>
          ...
          <div style={{ marginTop: 18, display: "flex", gap: 8 }}>
            ...
            <button type="button" className="outline" onClick={() => setForm({ alumnoId: "", cursoId: "", fecha: new Date().toISOString().slice(0, 10), estado: "PRESENTE", observacion: "" })}>
              Limpiar
            </button>
          </div>
        </form>
      ...
      {historial.length > 0 && (
        <div>
          ...
          <table>
            <thead>
              <tr><th>Alumno</th><th>Curso</th><th>Fecha</th><th>Estado</th><th>Observacion</th></tr>
            </thead>
            <tbody>
              {historial.map((h) => (
                <tr key={h.id}>
